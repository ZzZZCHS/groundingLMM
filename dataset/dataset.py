...
def gvla_custom_collate_fn(batch, tokenizer=None, use_mm_start_end=True, inference=False, local_rank=-1):
    # Initializing lists and counters
    image_path_list, global_enc_image_list, grounding_enc_image_list = [], [], []
    bboxes_list, conversation_list, masks_list = [], [], []
    label_list, resize_list, questions_list = [], [], []
    ...
    for (image_path, global_enc_image, grounding_enc_image, bboxes, conversations, masks, label, resize, questions,
        ...
        if not batch_meta:
            batch_meta = {k: {kk: [] for kk in v} if isinstance(v, dict) else [] for k, v in meta.items()}
        for k, v in meta.items():
            if isinstance(v, dict):  # If the value is a dictionary, merge its sub-keys
                for kk, vv in v.items():
                    batch_meta[k][kk].append(vv)
            else:
    ...
    for k, v in batch_meta.items():
        if isinstance(v, dict):  # If the value is a dictionary, stack its values
            for kk in v:
                batch_meta[k][kk] = torch.from_numpy(np.stack(v[kk], axis=0))
        else:
            batch_meta[k] = torch.from_numpy(np.stack(v, axis=0))
