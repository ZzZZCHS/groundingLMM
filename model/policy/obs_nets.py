...
from tools.python_utils import extract_class_init_kwargs_from_dict
import tools.tensor_utils as TensorUtils
import tools.obs_utils as ObsUtils
from model.policy.base_nets import Module, Sequential, MLP, RNN_Base, ResNet18Conv, SpatialSoftmax, \
    FeatureAggregator
from model.policy.obs_core import VisualCore, Randomizer, VisualCoreLanguageConditioned
from model.policy.transformers import PositionalEncoding, GPT_Backbone
...
class ObservationGroupEncoder(Module):
    ...
    def forward(self, **inputs):
        ...
        for obs_group in self.observation_group_shapes:
            ...
            outputs.append(
                self.nets[obs_group].forward(inputs[obs_group], inputs["mask_embeds"])
            )
...
class MIMO_Transformer(Module):
    ...
    def __init__(
        ...
        # hard code !!
        self.mask2obs_projector = nn.Linear(512, 64)
